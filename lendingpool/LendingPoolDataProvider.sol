...
contract LendingPoolDataProvider is VersionedInitializable {
    ...
    **/
    ...
    function getUserSaveInfo(address _user,address _reserve) public view returns(uint totalLiquidityBalanceTRX,uint totalBorrowBalanceTRX,uint cTokenLiquidityAmount,uint cTokenBorrowAmount){
        PriceOracle oracle = PriceOracle(addressesProvider.getPriceOracle());

        cTokenLiquidityAmount = CToken(core.getReserveCTokenAddress(_reserve)).balanceOf(_user);
        (, cTokenBorrowAmount, ) = core.getUserBorrowBalances(
            _reserve,
            _user
        );
   ...
   **/
    ...
    **/
    ...
    function calculateAvailableBorrowsTRXInternal(
        ...
        if (availableBorrowsTRX < borrowBalanceTRX.add(totalFeesTRX)) {
            return 0;
        }
        ...
        uint256 borrowFee = IFeeProvider(addressesProvider.getFeeProvider())
            .calculateLoanOriginationFee(address(0), availableBorrowsTRX);
